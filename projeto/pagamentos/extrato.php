...
$pagamentos = isset($_GET['pagamentos']) ? $_GET['pagamentos'] : "";

$mes_pagamentos = date('m');
$ano_pagamentos = date('Y');
$mes = $mes_pagamentos;
$ano = $ano_pagamentos;
...
<body>
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col">
                <h4>Extrato Pagamentos - <?php print "" . $mes_data . " " . $ano . ""; ?></h4>
            </div>
        </div><br>
        ...
        <table class="table table-hover table-sm">
            ...
            <tbody>
                ...
                //Consulta Banco
                $sql = "SELECT * FROM pagamentos WHERE MONTH(vencimento) = $mes AND YEAR(vencimento) = $ano AND $filtro LIKE '%$palavra%' ORDER BY vencimento";
                ...
                while ($exibirRegistros = mysqli_fetch_array($consulta)) {
                    ...
                    $despesa = $exibirRegistros[3];
                    $real_despesa = number_format($despesa, 2, ',', '.');
                    $categoria = $exibirRegistros[4];
                    $carteira = $exibirRegistros[5];
                    $anexo = $exibirRegistros[6];
                    $pago = $exibirRegistros[7];
                ...
                <th scope="col" class="linha">
                    <?php
                    $sql_despesa = "SELECT sum(despesa) FROM pagamentos WHERE MONTH(vencimento) = $mes AND YEAR(vencimento) = $ano AND $filtro LIKE '%$palavra%'";
                    $consulta_despesa = mysqli_query($conexao, $sql_despesa);
                    $exibir_despesa = mysqli_fetch_array($consulta_despesa);

                    $valor =  $exibir_despesa[0];
        ...
        <div class="row">
            <div class="col">
                <?php
                //Mostra Registros
                $sql = "SELECT * FROM pagamentos WHERE MONTH(vencimento) = $mes AND YEAR(vencimento) = $ano AND $filtro LIKE '%$palavra%'";
                $consulta = mysqli_query($conexao, $sql);
